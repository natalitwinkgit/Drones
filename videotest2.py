...
# Force OpenCV to use specific FFmpeg settings for Tello H.264
# This helps prevent the "grey/blue" blocky artifacts and timeouts on macOS
os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "protocol_whitelist;file,rtp,udp|fflags;nobuffer|flags;low_delay|probesize;32|analyzeduration;0"
...
class TelloVideoThread(QThread):
    ...
    def run(self):
        # Using a more robust URL string for macOS FFmpeg backend
        video_url = 'udp://@0.0.0.0:11111?overrun_nonfatal=1&fifo_size=50000000'
        
        # Give the drone a moment to start the hardware encoder
        self.msleep(2000)
        
        # Explicitly use the FFMPEG backend
        cap = cv2.VideoCapture(video_url, cv2.CAP_FFMPEG)
        
        # Crucial for low-latency UDP: set buffer to 1 frame
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        if not cap.isOpened():
            print("Error: Video stream failed to open. Check connection/firewall.")
            return

        print("Video stream connected. Decoding H.264...")
        ...
        while self.running:
            try:
                ...
                if ret and frame is not None:
                    ...
                    # rgbSwapped converts BGR -> RGB
                    q_img = QImage(frame.data, width, height, bytes_per_line, QImage.Format.Format_RGB888).rgbSwapped()
                    self.frame_received.emit(q_img)
                else:
                    # Small sleep if frame is dropped to prevent CPU spike
                    self.msleep(1)
...
class TelloFullPanel(QWidget):
    ...
    def initUI(self):
        ...
        self.terminal_display = QLabel(" > Connecting to Tello...")
